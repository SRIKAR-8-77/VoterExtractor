...
def background_worker_loop():
    ...
    # We MUST use a Manager() Queue when passing to ProcessPoolExecutor jobs instead of inheritance
    ...
    while True:
        ...
        try:
            ...
            try:
                future = pool.submit(process_pdf_isolated, task, state.master_mp_queue)
                
                # Attach a callback to catch silent crashes inside the Process
                def on_done(f):
                    try:
                        f.result()
                    except Exception as e:
                        # f.exception() will contain the traceback from the child process
                        crash_tb = "".join(traceback.format_exception(type(e), e, e.__traceback__)) if e.__traceback__ else str(e)
                        state.logs.append(f"{time.strftime('%H:%M:%S')} [FATAL] - Process crashed entirely for {task['filename']}: \n{crash_tb}")
                        # Also clean up the active jobs UI so it doesn't get stuck forever
                        if task['job_id'] in state.active_jobs:
                            state.active_jobs[task['job_id']]['stage'] = "CRASHED. See Logs."
